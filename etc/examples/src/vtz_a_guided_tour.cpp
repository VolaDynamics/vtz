...
int main() {
    ...
    // Let's display the current time, both in America/New_York, and in the
    // current timezone.
    //
    // Here, `tz->format()` converts a sys_time (measuring Unix time) to a local
    // time in the given zone. It uses the format specifiers for [`strftime`].
    //
    // Here, `%c` displays the date and time (eg, `Fri Feb 27 12:09:46 2026`),
    // while `%Z` displays the current timezone abbreviation (eg, `EST` or `EDT`
    // for Eastern Time, depending on the time of year).
    //
    // I'm in US Eastern Time, so for me, the output is the same in both cases.
    //
    // If you just want to display UTC time, you can use `vtz::format()`, which
    // formats in terms of UTC.
    //
    ...
    // `system_clock::now()` usually returns time in milliseconds or
    // microseconds (depending on the system), se we used `std::chrono::floor()`
    // to get a time in seconds in the above snippet.
    //
    // The format function will display up to 9 digits, but if the input time is
    // less precise, it uses fewer digits.
    //
    // [`std::chrono::high_resolution_clock`] can sometimes provide even more
    // precision, however it often uses [`steady_clock`] under the hood, which
    // does not necessarily return Unix Time.
    //
    // For this purpose, you should use [`clock_cast`] to convert it to a
    // `sys_time`.
    //
    // [`clock_cast`]:
    //   https://en.cppreference.com/w/cpp/chrono/clock_cast.html
    // [`steady_clock`]:
    //   https://en.cppreference.com/w/cpp/chrono/steady_clock.html
    ...
    // Let's parse a timestamp. vtz aims to be able to round-trip any format
    // specifiers supported by `vtz::format()` and `tz->format()`.
    ...
    // - `%Y` is the year
    //
    // If you wanted second precision, you could use `%T` instead of `%R`, and
    // then this would be equivalent to `%c`:

    sys_seconds T2 = vtz::parse<sec>( "%c", "Thu Feb 26 10:30:00 2026" );
    assert( T == T2 );

    // `vtz::parse` also accepts fractions of a second for any format specifiers
    // which handle seconds:
    ...
    //
    // If you wanted to write to a buffer, you could instead use
    // `tz->format_to()` instead.
    ...
    // `%z` represents an ISO 8601 standard timezone specification - this is the
    // offset expressed as `±hh[mm]`
    //
    // Because `T` does not fall during daylight savings time, the zone is 5
    // hours behind UTC, so this prints `UTC offset at T: -18000s (UTC-05)`
    ...
    //
    // The answer depends on the timezone, and the rules for that timezone at
    // the given input time. And it *also* determines the current time within
    // the zone, because for many time zones, the offset from UTC changes
    // throughout the year. When a given input time falls is therefore critical
    // for determining the local time.
    //
    // The rules for a timezone are also subject to change due to legislation,
    // and this dependence on social convention and governance is the reason the
    // tz database (and timezone libraries like vtz) exist at all - you want to
    // be able to answer this question reliably, for any time zone, based on a
    // standardized and publicly available source of truth.
    //
    // Yeah, ok. So when does it start? And how far off are we from UTC time?
    //
    // For a *specific* time, in a *specific* zone, this is easy to answer with
    ...
    // There we go. The current offset at T is -5h (-18000 seconds), the
    // abbreviation is `EST`, the save is 0 minutes (we're not in daylight
    // savings time), and at 01:59:59 AM EST we move the clocks forwards an
    // hour, and it becomes 03:00:00 AM EDT.
    //
    // That being said, in most cases you probably want a specific piece of
    // information, such as the current local time; the offset; or (if you
    // already have a local time) the UTC time.
    ...
    // Updating `T` to be the start of daylight savings time, we see that
    // daylight savings time lasts from 3AM EDT on the second Sunday in March,
    // to 1:59:59 AM EDT on the first Sunday in November.
    //
    // If we wanted, we could repeat this process of `info = tz->get_info(
    // info.end )` to enumerate all future switches between daylight time and
    // standard time (at least until `T` becomes too large to fit in a 64-bit
    // integer).
    ...
    // Converting a local time back to a sys time typically matches the
    // input sys_time, however local times are ambiguous whenever the clocks
    // fall back. In zones where this happens, this typically occurs for 1 hour,
    // once a year.
    //
    // Take a look at when daylight savings time ends in the above example.
    // For the US, this is currently the first Sunday in November, at what
    // _would_ be 2AM EDT.
    //
    // When 1:59:59AM EDT ends on that date, clocks fall back to 1AM EST, and
    // times between then are ambiguous: 1:30AM could refer to _either_ EDT or
    // EST.
    //
    // When converting to UTC time, this disambiguation should be made with
    // `vtz::choose` - `choose::earliest` returns the earliest system time,
    // and `choose::latest` returns the latest system time.
    //
    // (Most of the time these give the exact same result, but the difference
    // must be acknowledged)

    auto T_ambig = vtz::parse_local<sec>( "%c", "Sun Nov 01 01:30:00 2026" );

    // Here, there are two possible offsets: UTC-04 (EDT), and UTC-05 (EST)
    // These correspond to 05:30 UTC and 06:30 UTC, respectively.
    ...
    // Both of these map to the same local time, but the zone abbreviation and
